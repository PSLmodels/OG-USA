...
def get_macro_params():
    ...
    fred_data['Debt held by public'] = (
        fred_data['Debt held by public'] / 1000)

    # Separate quartely, monthly, and annual dataseries
    fred_data_q = fred_data[[
        'Debt held by public', 'Nominal GDP', 'Total gov transfer payments',
        'Social Security payments', 'Gov expenditures',
        'Gov interest payments','GDP Per Capita']].resample('Q').mean()
    ...
    # find g_y
    macro_parameters['g_y'] = fred_data_q['GDP Per Capita'].pct_change(
        periods=4, freq='Q').mean()
